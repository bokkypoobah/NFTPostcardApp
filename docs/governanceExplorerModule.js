const GovernanceExplorer = {
  template: `
    <div class="mt-5 pt-3">
      <b-card no-body header="Governance (WIP)" class="border-0" header-class="p-1">
  ...
  computed: {
    ...
    commonTokenList() {
      ...
      this.tokenPickerList.forEach(function(e) {
        // logInfo("GovernanceExplorer", "commonTokenList(" + e.symbol + ")");
  ...
  methods: {
    ...
    addTokensToList(list) {
      logInfo("GovernanceExplorer", "addTokensToList(" + JSON.stringify(list) + ")");
    ...
    removeTokenFromList(address, symbol) {
      logInfo("GovernanceExplorer", "removeTokenFromList(" + address + ", '" + symbol + "')?");
      this.$bvModal.msgBoxConfirm('Remove ' + symbol + ' from token list? This can be added back later', {
        ...
        .then(value1 => {
          if (value1) {
            logInfo("GovernanceExplorer", "removeTokenFromList(" + address + ")");
    ...
    resetTokenList() {
      logInfo("GovernanceExplorer", "resetTokenList()?");
    ...
    async checkTokenAddress(event) {
      logInfo("GovernanceExplorer", "checkTokenAddress(" + this.tokenInfo.address + ")");
      ...
      try {
        ...
        logInfo("GovernanceExplorer", "checkTokenAddress: " + JSON.stringify(tokenInfo));
      ...
      logInfo("GovernanceExplorer", "checkTokenAddress: " + JSON.stringify(this.tokenInfo));
    },
    getSomeTokens(fakeTokenAddress) {
      fakeTokenAddress = fakeTokenAddress.toLowerCase();
      logInfo("GovernanceExplorer", "getSomeTokens(" + JSON.stringify(fakeTokenAddress) + ")");
      this.$bvModal.msgBoxConfirm('Get 1,000 ' + this.tokenData[fakeTokenAddress].symbol + ' tokens from the faucet for testing?', {
        ...
        .then(value1 => {
          if (value1) {
            logInfo("GovernanceExplorer", "getSomeTokens(" + this.tokenData[fakeTokenAddress].symbol + ")");
            var factoryAddress = store.getters['optinoFactory/address']
            logInfo("GovernanceExplorer", "getSomeTokens(" + fakeTokenAddress + ")");
            web3.eth.sendTransaction({ to: fakeTokenAddress, from: store.getters['connection/coinbase'] }, function(error, tx) {
                logInfo("GovernanceExplorer", "getSomeTokens() DEBUG2");
              if (!error) {
                logInfo("GovernanceExplorer", "getSomeTokens() token.approve() tx: " + tx);
                store.dispatch('connection/addTx', tx);
              } else {
                logInfo("GovernanceExplorer", "getSomeTokens() token.approve() error: ");
                console.table(error);
    ...
    setAllowance(address, decimals, newAllowance) {
      logInfo("GovernanceExplorer", "setAllowance(" + address + ", " + decimals + ", " + newAllowance + ")?");
      this.$bvModal.msgBoxConfirm('Set allowance for factory to transfer ' + this.newAllowance + ' tokens?', {
        ...
        .then(value1 => {
          if (value1) {
            logInfo("GovernanceExplorer", "setAllowance(" + address + ", " + decimals + ", " + newAllowance + ")");
            var factoryAddress = store.getters['optinoFactory/address']
            logInfo("GovernanceExplorer", "setAllowance() factoryAddress=" + factoryAddress);
            var token = web3.eth.contract(ERC20ABI).at(address);
            var allowance = new BigNumber(newAllowance).shift(decimals);
            logInfo("GovernanceExplorer", "setAllowance() allowance=" + allowance);

            var data = token.approve.getData(factoryAddress, allowance.toString());
            logInfo("GovernanceExplorer", "data=" + data);

            token.approve(factoryAddress, allowance.toString(), { from: store.getters['connection/coinbase'] }, function(error, tx) {
                logInfo("GovernanceExplorer", "setAllowance() DEBUG2");
              if (!error) {
                logInfo("GovernanceExplorer", "setAllowance() token.approve() tx: " + tx);
                store.dispatch('connection/addTx', tx);
              } else {
                logInfo("GovernanceExplorer", "setAllowance() token.approve() error: ");
                console.table(error);
                store.dispatch('connection/setTxError', error.message);
              }
            });
    ...
    async timeoutCallback() {
      // logInfo("GovernanceExplorer", "timeoutCallback() count: " + this.count);
      var tokenToolz = web3.eth.contract(TOKENTOOLZABI).at(TOKENTOOLZADDRESS);
      ...
      if (this.count == 0) {
        ...
        // logInfo("GovernanceExplorer", "timeoutCallback() - tokenPickerTotalRows: " + this.tokenPickerTotalRows);
        ...
        for (var i = 0; i < COMMONTOKENLIST.length; i++) {
          ...
          // logInfo("GovernanceExplorer", "timeoutCallback() - loading " + this.tokenPickerLoadingRow + " of " + this.tokenPickerTotalRows + " " + symbol);
        }
        ...
        logDebug("GovernanceExplorer", "timeoutCallback() - loaded " + this.tokenPickerTotalRows);
        ...
        logDebug("GovernanceExplorer", "timeoutCallback() - refreshed " + addressesLength);
        // logInfo("tokensModule", "timeoutCallback() tokenPickerList: " + JSON.stringify(this.tokenPickerList));
      }
...
const governanceExplorerModule = {
  namespaced: true,
  state: {
    params: null,
    executing: false,
  ...
  mutations: {
    deQueue (state) {
      logDebug("governanceExplorerModule", "deQueue(" + JSON.stringify(state.executionQueue) + ")");
      state.executionQueue.shift();
    },
    updateParams (state, params) {
      state.params = params;
      logDebug("governanceExplorerModule", "updateParams('" + params + "')")
    },
    updateExecuting (state, executing) {
      state.executing = executing;
      logDebug("governanceExplorerModule", "updateExecuting(" + executing + ")")
    },
  },
  actions: {
  },
